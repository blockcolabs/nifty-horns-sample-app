...
const $collection = atomFamily({
  key: "INIT::COLLECTION::STATE",
  default: null,
})

const $collectionStatus = atomFamily({
  key: "INIT::COLLECTION::STATUS",
  default: PROCESSING,
})

export function useInit(address) {
  const [collection, setCollection] = useRecoilState($collection(address))
  const [status, setStatus] = useRecoilState($collectionStatus(address))
  ...
  async function check() {
    ...
    // throwing an error.
    if (address != null) await isInitialized(address).then(setCollection)
    setStatus(IDLE)
  }
